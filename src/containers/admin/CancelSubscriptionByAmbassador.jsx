...
const CancelSubscriptionByAmbassador = () => {
  ...
  const handleSubmit = (values, { resetForm }) => {
    ...
    let urls = "subscription-cancelledby-ambassador";
  ...
  const handleDownloadReport = () => {
    const formattedData = orderDataSet.map(({ Ambassador_firstname, Ambassador_lastname, referral_code, subscription_cancellation_date }) => [
    ...
    const csvContent = [
      ["AMBASSADOR FIRST NAME", "AMBASSADOR LAST NAME", "AMBASSADOR REFERRAL CODE", "DATE OF SUBSCRIPTION CANCELLATION"],
      ...formattedData
    ]
      .map(row => row.join(","))
      .join("\n");
    ...
    saveAs(blob, "subscription_cancelled_by_ambassador.csv");
  };
          ...
          <main className="flex-1 overflow-y-auto pt-2 px-2  bg-base-200">
            ...
            <div className="bg-zinc-50 px-3 py-3 rounded-xl bg-white shadow-mx border border-zinc-200">
              <div className="text-xl font-semibold py-1 px-2">
                Subscription cancelled by Ambassador
              </div>
...
export default CancelSubscriptionByAmbassador;
