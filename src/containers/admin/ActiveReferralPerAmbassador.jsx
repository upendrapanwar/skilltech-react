...
const ActiveReferralPerAmbassador = () => {
  ...
  const handleSubmit = (values, { resetForm }) => {
    ...
    let urls = "active-referral-per-ambassador";
  ...
  const handleDownloadReport = () => {
    const formattedData = orderDataSet.map(({ Subscriber_firstname, Subscriber_lastname, referral_code, Date_of_use_of_referral_code, HVG_Subscription_status }) => [
    ...
    const csvContent = [
      ["SUBSCRIBER FIRST NAME", "SUBSCRIBER LAST NAME", "AMBASSADOR REFERRAL CODE USED", "DATE OF USE OF REFERRAL USED", "HVG SUBSCRIPTION STATUS"],
      ...formattedData
    ]
      .map(row => row.join(","))
      .join("\n");
    ...
    saveAs(blob, "active_referral_per_ambassador.csv");
  };
          ...
          <main className="flex-1 overflow-y-auto pt-2 px-2  bg-base-200">
            ...
            <div className="bg-zinc-50 px-3 py-3 rounded-xl bg-white shadow-mx border border-zinc-200">
              <div className="text-xl font-semibold py-1 px-2">
                Active Referrals per Ambassador
              </div>
...
export default ActiveReferralPerAmbassador;
